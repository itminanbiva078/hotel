        ...
        <div class="card card-default">
            <div class="card-header">
                ...
                <div class="card-tools">
                    @if(helper::roleAccess('salesTransaction.salePayment.index'))
                    <a class="btn btn-default" href="{{ route('salesTransaction.salePayment.index') }}"><i
                            class="fa fa-list"></i>
            ...
            <form class="needs-validation" method="POST"  action="{{ route('salesTransaction.salePayment.store') }}" novalidate>
            <!-- /.card-header -->
            <div class="card-body">
...
@section("scripts")
<script>
    $(document).on('change', '.customer_id,.collection_type', function() {
        let customer_id = $(this).val();
        let collection_type = $('.collection_type').val();
        $.ajax({
                "url": '<?php echo route("salesTransaction.salePayment.dueVoucherList"); ?>',
                "dataType": "json",
                "type": "GET",
                "data": {
                    "_token": "<?= csrf_token() ?>",
                    "customer_id": customer_id,
                    "collection_type": collection_type,//import save route
                }
                }).done(function(data) {
