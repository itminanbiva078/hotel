    ...
    <tbody>
        ...
        @foreach($reports as $key => $report)
            ...
            <tr>
                ...
                <td>
                    <a target="_blank" href="{{ route('salesTransaction.sales.show', $report->id) }}" show_id="{{ $report->id }}" title="Details" class="btn btn-xs btn-default  uniqueid{{ $report->id }}"><i class="fa fa-search-plus"></i>   {{$report->voucher_no}}</a>
                </td>    
